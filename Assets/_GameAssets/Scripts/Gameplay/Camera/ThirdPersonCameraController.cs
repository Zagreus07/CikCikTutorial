...
public class ThirdPersonCameraController : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        Vector3 viewDirection =
            _playerTransform.position - new Vector3(transform.position.x,
            _playerTransform.position.y, transform.position.z);

        _orientationTranform.forward = viewDirection.normalized;

        float horizontalInput = Input.GetAxisRaw("Horizontal");
        float verticalInput = Input.GetAxisRaw("Vertical");

        Vector3 inputDirection = _orientationTranform.forward * verticalInput + _orientationTranform.right * horizontalInput;
